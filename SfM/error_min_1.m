...
nonlcon = @(xv)cameraRot(xv, x, x_w, K , c, Ra, ra);
A = [];
b = [];
Aeq = [];
beq = [];
...
[x, obj, info, iter, nf, lambda] = sqp(init, fun, nonlcon, [], lb, ub);
end

function ceq = cameraRot(xv, uv, x_w, K , c, Ra, ra)
ceq = zeros(3+size(uv,1),1);
ceq(1,1) = xv(1)^2 + xv(2)^2 + xv(3)^2 -1;
ceq(2,1) = xv(4)^2 + xv(5)^2 + xv(6)^2 -1;
ceq(3,1) = xv(1)*xv(4) + xv(2)*xv(5) + xv(3)*xv(6);
r1 = [xv(1) xv(2) xv(3)];
r2 = [xv(4) xv(5) xv(6)];
r1 = r1/norm(r1);
r2 = r2/norm(r2);
