...
prevBearing = [ro xs];
v(:,:,1) = bearing_vec1;
view = addview(prevPoints, prevLabels, eye(3), zeros(1,3), prevBearing,1);
...
for i = 2:m
    ...
    [matchedVector1, matchedVector2, indexPairs] = matchVectors...
     ...
     view = addview(currPoints, currLabels, rot, trans, currBearing,i);
 ...
 %t_est = Out(:,4);
    ...
    xw_total = zeros(points_num, 3);
    xw = zeros(points_num, 3);
    [rows cols] = find(matchedPairs(:,1)> 0);
    for k = 1: i-1
      xyzPoints(i-1).points = triangulate(matchedVector1, matchedVector2,rot, trans);
      [rows cols] = find(indexPairs > 0);
      xw(rows,:) = xyzPoints(i-1).points;
      xw_total = xw_total + xw;
    end
    xw_average = sum(xw_total, 3)./sum(matchedPairs,2);
    out = optim_point(xw_average, view, m, n, matchedPairs);
    xw_est = reshape(out(1:3*points_num),[points_num, 3]);
    Rt_optim = reshape(out(3*points_num+1:end),[4*m, 3]);
    updateView();
