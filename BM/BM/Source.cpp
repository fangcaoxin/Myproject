...
int main(int argc, char** argv)
{
	...
	else if (filter == "wls_no_conf")
	{
		...
		if (algo == "bm")
		{
			...
			wls_filter->setDepthDiscontinuityRadius((int)ceil(wsize));//0.33*wsize
		...
		else if (algo == "sgbm")
		{
			...
			ROI = computeROI(left_for_matcher.size(), matcher);
			wls_filter = createDisparityWLSFilterGeneric(false);
			wls_filter->setDepthDiscontinuityRadius((int)ceil(wsize)); //0.5*wsize
	...
	if (!no_display)
	{
		...
		//! [visualization]
		Mat raw_disp_vis_left;
		Mat raw_disp_vis_right;
		normalize(left_disp, raw_disp_vis_left, 0, 255, NORM_MINMAX, CV_8U);
		//getDisparityVis(left_disp, raw_disp_vis_left, vis_mult);
		namedWindow("raw disparity left", WINDOW_AUTOSIZE);
		imshow("raw disparity left", raw_disp_vis_left);
		/*getDisparityVis(right_disp, raw_disp_vis_right, vis_mult);
		namedWindow("raw disparity right", WINDOW_AUTOSIZE);
		imshow("raw disparity right", raw_disp_vis_right);*/
		Mat filtered_disp_vis;
		normalize(filtered_disp, filtered_disp_vis, 0, 255, NORM_MINMAX, CV_8U);
		//getDisparityVis(filtered_disp, filtered_disp_vis, vis_mult);
