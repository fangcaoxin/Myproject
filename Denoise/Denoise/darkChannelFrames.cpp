...
void sumAreaByRadius(vector<Mat>& diff_wb, Mat& sum, int radius) {
	...
	for (int k =1 ; k < centroids[0].rows; k++) {
		int i = centroids[0].at<int>(k, 1);
		int j = centroids[0].at<int>(k, 0);
		st_row = i - radius, ed_row = i + radius;
		st_col = j - radius, ed_col = j + radius;
		...
		for (int m = st_row; m <= ed_row; m++)
		{
			for (int n = st_col; n <= ed_col; n++)
			{
				if (labels[1].at<int>(m, n) != 0) {
					...
					double dis = (centroids[1].at<int>(label, 1) - i)*(centroids[1].at<int>(label, 1) - i) + (centroids[1].at<int>(label, 0) - j)*(centroids[1].at<int>(label, 0) - j);
