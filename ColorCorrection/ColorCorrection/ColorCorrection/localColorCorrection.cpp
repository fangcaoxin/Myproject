...
void localColorCorrection(Mat& src, Mat& dst) {
	...
	imshow("attenation map", attenuation_map);
	...
	Mat color_corrected;
	color_corrected_list[0] = attenuation_map.mul(composited_split[0]) + r_pow.mul(bgr[0]);
	color_corrected_list[1] = attenuation_map.mul(composited_split[1]) + r_pow.mul(bgr[1]);
	color_corrected_list[2] = attenuation_map.mul(composited_split[2]) + r_pow.mul(bgr[2]);
